if IY_LOADED and not _G.IY_DEBUG then
	-- error("Bob Hub is already running!", 0)
	return
end
...
Title.TextSize = 18
Title.Text = "Bob Hub FE v" .. currentVersion
...
PluginsHint.Text = "Download plugins from the IY Discord (https://discord.gg/yTt3ZNEpAC)"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/yTt3ZNEpAC")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/yTt3ZNEpAC)"
	end)
	dragGUI(main)
	main.Parent = ScaledHolder
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Bob Hub Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Bob Hub discord server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.gg/yTt3ZNEpAC')
		notify('Discord Invite', 'Copied to clipboard!\nhttps://discord.gg/yTt3ZNEpAC')
	else
		notify('Discord Invite', 'https://discord.gg/yTt3ZNEpAC')
	end
	if httprequest then
		httprequest({
...
addcmd('keepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		notify('KeepIY','Bob Hub will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		notify('KeepIY','Bob Hub will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
